...
# 도서관 데이터 로드
@st.cache_data
def load_library_data(library_csv_path):
    return preprocess_library_data(library_csv_path)

# 사이드바에 도서관 데이터 경로 추가
library_data_path = st.sidebar.text_input("도서관 데이터 파일 경로:", "library.csv")

try:
    library_df = load_library_data(library_data_path)
    st.sidebar.success(f"도서관 데이터 로드 완료: {len(library_df)}개 도서관 정보")
except Exception as e:
    st.sidebar.warning(f"도서관 데이터 로드 실패: {e}")
    library_df = None
...
def create_recommendation_prompt(user_request, top_candidates_df, library_info=None):
    """
    ...
    1. 위 도서 목록 중에서 **사용자 요청에 가장 적합한 3권**을 선택하세요.
    ...
    if library_info:
        ...
        또한, 사용자 위치 근처의 도서관 정보도 추천과 함께 언급해주세요:
...
def run_hybrid_recommendation(recommendation_system, user_request, library_df=None, model="gpt-4o", num_candidates=15):
    """
    ...
    if library_df is not None:
        user_location = extract_location_from_request(user_request)
        if user_location:
            nearby_libraries = find_nearby_libraries(user_location, library_df)
            if nearby_libraries is not None:
                library_info = format_library_info(nearby_libraries)
    ...
    # LLM 프롬프트 생성(도서관 정보 포함)
    prompt = create_recommendation_prompt(user_request, top_candidates, library_info)
    ...
    return {
        ...
        "user_location": user_location if 'user_location' in locals() else None
    }
    ...
    if st.button("🤖 하이브리드 추천 받기 (ML + LLM)", use_container_width=True):
        with st.spinner(f"하이브리드 추천 생성 중 😁 (30초 이상 소요될 수 있습니다.)"):
            try:
                ...
                # 위치 정보가 있으면 표시
                if results.get("user_location"):
                    st.markdown(f"### 📍 추출된 위치 정보: {results['user_location']}")
                
                st.markdown("### 🤖 LLM 추천 결과")
                st.markdown(results["recommendation"])
                
                # 도서관 정보가 있으면 별도로 표시
                if results.get("library_info"):
                    with st.expander("📚 인근 도서관 정보 보기"):
                        st.markdown(results["library_info"])
